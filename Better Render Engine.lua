...
local Long = ui.new_slider("AA", "Anti-aimbot angles", "Long", 0, 1000, 50, true, "")
local Width = ui.new_slider("AA", "Anti-aimbot angles", "Width", 0, 1000, 50, true, "")
local circle = ui.new_slider("AA", "Anti-aimbot angles", "Circle", 0, 40, 5, true, "")
local glow_radius = ui.new_slider("AA", "Anti-aimbot angles", "Circle", 0, 200, 15, true, "")
local downhalf_label = ui.new_label( "AA", "Anti-aimbot angles", "Color Picker For Container")
local downhalf = ui.new_color_picker( "AA", "Anti-aimbot angles", "Color Picker For Container", 17, 17, 17, 80)
local m_render_engine = (function()
	...
	local container_uphalf = function(x, y, long, width, round, color_r, color_g, color_b, alpha, alpha_glow)
		...
		renderer_circle_outline(x + long - round, y + round, color_r, color_g, color_b, alpha, round, 270, 0.25, 2)
        --Left/Right Gradient
		renderer_gradient(x, y + round, 2, width - round * 2, color_r, color_g, color_b, alpha, color_r, color_g, color_b, n, false)
		renderer_gradient(x + long - 2, y + round, 2, width - round * 2, color_r, color_g, color_b, alpha, color_r, color_g, color_b, n, false)
        --Down left/right circle
		renderer_circle_outline(x + round, y + width - round, color_r, color_g, color_b, n + 50, round, 90, 0.25, 2)
		renderer_circle_outline(x + long - round, y + width - round, color_r, color_g, color_b, n + 50, round, 0, 0.25, 2)
        --Main line from down
	...
	a.render_container = function(x, y, long, width, color_red, color_green, color_blue, alpha, w)
        local ind_r, ind_g, ind_b, ind_a = ui.get(downhalf)
		renderer.blur(x, y, long, width, 100, 100)
		container_main(x, y, long, width, Round + ui.get(circle), ind_r, ind_g, ind_b, ind_a)
		container_uphalf(x, y, long, width, Round + ui.get(circle), color_red, color_green, color_blue, alpha, o + ui.get(glow_radius))
		if w then
...
client.set_event_callback(
    "paint",
    function(e)
        m_render_engine.render_container(center_x + 2, center_y + 46, ui.get(Long), ui.get(Width), 255, 255, 255, 255)
    end)